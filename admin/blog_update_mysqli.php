...
if(isset($_GET['article_id']) && !$_POST){
	//prepare SQL query
	$sql='SELECT article_id, image_id, title, article FROM blog where article_id= ?';
	if($stmt->prepare($sql)){
		...
		$stmt->fetch();
		//free the database resource for the second query
		$stmt->free_result();
		//get categories associated with the article
		$sql='SELECT cat_id FROM acticle2cat WHERE article_id = ?';
		if($stmt->prepare($sql)){
			$stmt->bind_param('i', $_GET['article_id']);
			$OK=$stmt->execute();
			$stmt->bind_result($cat_id);
			//loop through the results to store than in an array
			$selected_categories=[];
			while($stmt->fetch()){
...
if(isset($_POST['update'])){
	...
	if(!empty($_POST['image_id'])){
		$sql='UPDATE blog SET image_id=?, title=?, article=? WHERE article_id = ?';
		if($stmt->prepare($sql)){
			$stmt->bind_param('issi', $_POST['image_id'], $_POST['title'], $_POST['article'], 
				$_POST['article_id']);
			$done=$stmt->execute();
		}
	}else{
		$sql='UPDATE blog SET image_id=NULL, title=?, article=? WHERE article_id = ?';
		if($stmt->prepare($sql)){
	...
	//delete existing values in the cross-reference table
		...
		//insert the new values in article2cat
...
if($done || !isset($_GET['article_id'])){
	header('Location: http://phpsols/admin/blog_list_mysqli.php');
	exit;
}
