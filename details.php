...
//connect to database
$conn=dbConnect('read');
//check the article id in the query string
if(isset($_GET['article_id']) && is_numeric($_GET['article_id'])){
    $article_id=(int) $_GET['article_id'];
}else{
    $article_id=0;
}
$sql="SELECT title, article,DATE_FORMAT(updated, '%W, %M% %D, %Y') 
       AS updated, filename, caption 
...
<div id="wrapper">
    ...
    <main>
        ...
        <p><a href="
            <?php 
            //check that browser supports $_SERVER define_syslog_variables()
            if(isset($_SERVER['HTTP_REFERRER']) && isset($_SERVER['HTTP_HOST']))
            {
                $url=parse_url($_SERVER['HTTP_REFERRER']);
